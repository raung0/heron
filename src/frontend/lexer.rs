			...
			'\'' => {
				...
				if c1 != '\\' && c2 == '\'' {
					...
					if self.ch == 'x' {
						...
						let mut n = 0;
						while n < 3
							&& self.ch.is_ascii_digit() && self.ch < '8'
						{
							esc.push(self.ch);
							self.advance();
							n += 1;
	...
	fn read_number(&mut self) -> Result<Token, LexerError> {
		...
		if self.ch == '0' {
			...
			if n1 == 'x' || n1 == 'X' {
				...
				let val = u64::from_str_radix(raw.trim_start_matches("0x"), 16)
					.unwrap_or(0);
				...
				let val = u64::from_str_radix(raw.trim_start_matches("0b"), 2)
					.unwrap_or(0);
				self.finalize_location(&mut src);
				return Ok(Token {
					location: src,
				...
				let val = u64::from_str_radix(raw.trim_start_matches("0o"), 8)
					.unwrap_or(0);
				self.finalize_location(&mut src);
				return Ok(Token {
					location: src,
